...
const editProfile = async (req, res) => {
    const { id, full_name, nik, phone, date_of_birth, marital_status, address } = req.body;

    if (!id || !full_name || !nik || !phone || !date_of_birth || !marital_status || !address) {
        return res.render('admin/profile', {
            site_title: 'Profile | E-Manager',
            user: req.user,
    ...
    try {
        const query = 'UPDATE users SET full_name = ?, nik = ?, phone = ?, date_of_birth = ?, marital_status = ?, address = ? WHERE id = ?';
        await db.execute(query, [full_name, nik, phone, date_of_birth, marital_status, address, id]);
